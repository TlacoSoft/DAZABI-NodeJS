...
router.get('/count/:id', (req, res) => {
    const { id } = req.params; 
    mysqlConnection.query('Select count(*) as recompensa from recompensa_usuario where idusuario = ?', [id], (err, rows, fields) => {
      if (!err) {
        if(rows!=''){
          res.json(rows[0]);  
        }else{                
...
router.get('/get/:id', (req, res) => {
    const { id } = req.params; 
    mysqlConnection.query('SELECT * FROM recompensa_usuario INNER JOIN recompensas on recompensas.idRecompensa = recompensa_usuario.idRecompensa WHERE idusuario = ?', [id], (err, rows, fields) => {
      if (!err) {
        if(rows!=''){
          res.json(rows);  
        }else{                
